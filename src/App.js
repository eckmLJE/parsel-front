...
class App extends Component {
  constructor() {
    ...
    this.state = {
      ...
      currentAnnotations: [
        { id: "1001", start: 41, end: 116, content: "A powerful statement in its simplicity" }, // one of the most disgraceful performances by an American president in memory
        { id: "1003", start: 393, end: 448, content: "This could literally be true" }, // He and Putin seemed to be speaking from the same script
        { id: "1002", start: 251, end: 315, content: "Think of his entire team that is supporting this" }, // But it is clear that the summit in Helsinki was a tragic mistake
        { id: "1004", start: 419, end: 489, content: "A conscious choice? If he's not suffering from dementia" } // speaking from the same script as the president made a conscious choice
      ],
      annotationId: 1004,
      currentHighlights: [],
      statementArray: []
  ...
  makeStatementArray = () => {
    ...
    highlights.forEach(highlight => {
      newStatementArray.push(
        <TextFragment key={newStatementArray.length} content={statement.slice(charCounter, highlight.start)} />
      );
      newStatementArray.push(
        <HighlightSpan
          content={statement.slice(highlight.start, highlight.end)}
          name={highlight.name} key={highlight.name}
        />
      );
